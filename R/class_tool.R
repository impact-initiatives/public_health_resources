...
Tool <- R6::R6Class(
  ...
  public = list(
    ...
    set_name = function(new_name) {
      if (
        !is.character(new_name) || length(new_name) != 1 || nchar(new_name) == 0
      ) {
    ...
    change_default_language = function(language) {
      ...
      if (
        !is.character(language) || length(language) != 1 || nchar(language) == 0
      ) {
      ...
      language_title <- paste0(
        toupper(substring(language_norm, 1, 1)),
    ...
    filter_survey_by_indicator = function(indicator_codes) {
      ...
      self$revised_choices <- private$.filter_choices_from_survey(
        all_filter_codes
      )
    ...
    update_choice_list = function(list_name, new_choices) {
      ...
      target <- if (
        !is.null(self$revised_choices) && nrow(self$revised_choices) > 0
      ) {
    ...
    add_numbered_list = function(list_name, prefix_val, prefix_lbl, n) {
      ...
      new_rows <- lapply(seq_len(n), function(i) {
        ...
        for (col in setdiff(names(choices), names(row))) {
          row[[col]] <- NA
        }
        row
      })
      ...
      for (col in setdiff(all_cols, names(choices))) {
        choices[[col]] <- NA
      }
      for (col in setdiff(all_cols, names(new_df))) {
        new_df[[col]] <- NA
      }
      self$revised_choices <- rbind(
        choices[, all_cols, drop = FALSE],
        new_df[, all_cols, drop = FALSE]
      )
      self$touch()
      invisible(self)
    },
    ...
    get_indicator_codes = function(prefer_revised = TRUE) {
      sv <- if (
        prefer_revised &&
          ...
          is.data.frame(self$revised_survey) &&
          nrow(self$revised_survey) > 0
      ) {
        self$revised_survey
      } else {
        self$survey
    ...
    get_filtered_survey = function(indicator_mapping = NULL) {
      if (
        is.null(indicator_mapping) || length(private$.selected_indicators) == 0
      ) {
        return(self$survey)
      }

      # Get question names for selected indicators
      selected_questions <- unlist(indicator_mapping[
        private$.selected_indicators
      ])
    ...
    get_filtered_choices = function(indicator_mapping = NULL) {
      ...
      select_types <- filtered_survey$type[grepl(
        "^select_",
        filtered_survey$type
      )]
  ...
  private = list(
    ...
    .validate_survey_structure = function(survey) {
      result <- xlsform_check_required_sheet_cols(
        survey,
      ...
      if (!result$valid) {
        ...
        stop(paste(
          "Survey sheet missing required columns:",
          paste(msgs, collapse = "; ")
        ))
      }
    },
    ...
    .validate_choices_structure = function(choices) {
      ...
      if (!result$valid) {
        msgs <- vapply(result$issues, function(i) i$message, character(1L))
        stop(paste(
          "Choices sheet missing required columns:",
          paste(msgs, collapse = "; ")
        ))
      }
    },
    ...
    .validate_survey_content = function() {
      ...
      if ("type" %in% names(sv)) {
        ...
        for (i in seq_len(nrow(sv))) {
          raw_type <- sv$type[i]
          base_type <- tolower(trimws(gsub(
            "\\s+.*$",
            "",
            as.character(raw_type)
          )))
          if (is.na(base_type) || base_type %in% meta_types) {
            next
          }
    ...
    .validate_choices_content = function() {
      ...
      for (list_nm in unique(ch$list_name)) {
        ...
        if ("name" %in% names(list_ch)) {
          dup_names <- list_ch$name[
            duplicated(list_ch$name) & !is.na(list_ch$name)
          ]
          if (length(dup_names) > 0) {
            msg <- paste0(
              "Duplicate choice names in list '",
              list_nm,
              "': ",
    ...
    .validate_revised_coherence = function() {
      sv <- if (
        !is.null(self$revised_survey) && nrow(self$revised_survey) > 0
      ) {
        self$revised_survey
      } else {
        self$survey
    ...
    .add_question = function(new_row, after = NULL, before = NULL) {
      ...
      for (col in setdiff(all_cols, names(sv))) {
        sv[[col]] <- NA
      }
